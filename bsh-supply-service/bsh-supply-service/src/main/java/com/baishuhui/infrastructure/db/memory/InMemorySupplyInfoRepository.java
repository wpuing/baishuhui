...
public class InMemorySupplyInfoRepository implements ISupplyInfoRepository {
    ...
    public List<SupplyInfo> findBrowsableFiltered(String category, String locationKeyword, String status, int limit) {
        ...
        String loc = locationKeyword == null ? "" : locationKeyword.trim().toLowerCase();
        String st = status == null ? "" : status.trim();
        return store.values().stream()
                .filter(s -> {
                    if (!st.isEmpty()) {
                        return st.equals(s.getStatus());
                    }
                    return !SupplyInfo.DRAFT.equals(s.getStatus()) && !SupplyInfo.CANCELLED.equals(s.getStatus());
                })
                .filter(s -> cat.isEmpty() || cat.equals(s.getSpec().category()))
